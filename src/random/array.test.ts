...
import { U16Array, U32Array, U8Array } from "./array.ts";
import * as secure from "./secure/array.ts";

Deno.test("randomization mutates the underlying buffer", () => {
	const len = 1024;

	const u8Arr = U8Array.ofLength(len);
	...
	const u8ArrSecure = secure.U8Array.ofLength(len);
	...
	const u16Arr = U16Array.ofLength(len);
	...
	const u16ArrSecure = secure.U16Array.ofLength(len);
	...
	const u32Arr = U32Array.ofLength(len);
	...
	const u32ArrSecure = secure.U32Array.ofLength(len);
...
Deno.test("string formatting is correct", () => {
	const u8Arr = U8Array.ofLength(2);
	...
	const u16Arr = U16Array.ofLength(2);
	...
	const u32Arr = U32Array.ofLength(2);
