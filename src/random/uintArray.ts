...
/**
 * The `SecureRandomU16Array` class provides utilities for representing
 * and encoding cryptographically secure random 16-bit unsigned integer arrays in
 * various string formats.
 *
 * @example
 * ```typescript
 * const r = SecureRandomU16Array.ofLength(2);
 * // or
 * // const r = new SecureRandomU16Array(2);
 * console.log(r.toHex()); // "ff10"
 * ```
 *
 * @remarks String output does **not** include prefix (e.g., `0b`, `0o`, `0x`).
 */
export class SecureRandomU16Array extends Uint16Array {
	static ofLength(length: number): SecureRandomU16Array {
		return new SecureRandomU16Array(length);
	}
	...
	/**
	 * @remarks **Mutates the underlying buffer.**
	 * @returns the same SecureRandomU16Array with each element randomized to a new random u16 value.
	 */
	randomize(): this {
		return crypto.getRandomValues(this);
	}
...
/**
 * The `SecureRandomU32Array` class provides utilities for representing
 * and encoding cryptographically secure random 4-byte
 * arrays in various string formats.
 *
 * @example
 * ```typescript
 * const r = SecureRandomU32Array.ofLength(2);
 * // or
 * // const r = new SecureRandomU32Array(2);
 * console.log(r.toHex()); // "ff10"
 * ```
 *
 * @remarks String output does **not** include prefix (e.g., `0b`, `0o`, `0x`).
 */
export class SecureRandomU32Array extends Uint32Array {
	static ofLength(length: number): SecureRandomU32Array {
		return new SecureRandomU32Array(length);
	}
	...
	/**
	 * @remarks **Mutates the underlying buffer.**
	 * @returns the same SecureRandomU32Array with each element randomized to a new random u32 value.
	 */
	randomize(): this {
		return crypto.getRandomValues(this);
	}
